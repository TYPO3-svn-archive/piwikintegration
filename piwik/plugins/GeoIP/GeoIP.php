...
{	
	public function getInformation()
	{
		$info = array(
			'name' => 'GeoIP',
			'description' => 'GeoIP // get user precise localisation from its IP',
	...
	public function install()
	{
		...
		$query = "ALTER IGNORE TABLE `".Piwik::prefixTable('log_visit')."` " .
...
//		$this->archiveDayAggregateGoals($archiveProcessing);
	...
	protected function getLocationInfo($ip)
	{
		...
		if(isset($record->country_code))	$locationInfo['country_code'] = strtolower($record->country_code);
	...
	{
		require_once "GeoIP/libs/geoipcity.inc";
	...
	public function updateExistingVisitsWithGeoIpData()
	{
		...
		$stmt = $db->query("SELECT count(*) as cnt FROM ".Piwik::prefixTable('log_visit'), array() );
		$row = $stmt->fetch();
		...
		$this->initGeoIpDatabase();
		Piwik::log("$count rows to process in ".Piwik::prefixTable('log_visit')."...");
		flush();
		while( $start < $count )
		{
